...
tc AS (
  ...
  AND closing_date BETWEEN '{period_start:%Y-%m-%d}' AND '{period_end:%Y-%m-%d}'
), cs AS (
  ...
  WHERE 1
  AND s.enabled = 1
  AND s.status IN (1, 2, -1, -2, -3, -4, -5) -- adding active sales
  AND a.borough IN (100,200,300,400,500)
  AND active_date BETWEEN DATEADD(DAY, {day_lim[0]}, '{period_start:%Y-%m-%d}') AND DATEADD(DAY, {day_lim[1]}, '{period_end:%Y-%m-%d}')
 ...
 AND s.addr_lat BETWEEN 40.486608 AND 40.923248
    ...
    FROM tc left join cs ON
        ST_DWITHIN(tc.point, cs.point, {distance})
        AND ((tc.closing_price / cs.price ) - 1) BETWEEN {price_lim[0]} AND {price_lim[1]} -- price diff range
        AND days BETWEEN {day_lim[0]} AND {day_lim[1]} -- time diff
), scount AS (
   select sale_id,
          COUNT(*) as candidates
   FROM c
